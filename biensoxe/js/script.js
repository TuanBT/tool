...
function showListProvince () {
  ...
  var posEdgeRightMap = divMap.position().left + divMap.width()
  divListPro1.css({ 'left': posEdgeRightMap })
  ...
  for (var i = 0; i < codeProvince.length; i++) {
    for (var j = 0; j < codeProvince[i].code.length; j++) {
      if (countNumLine > numLine) {
        ...
        $('#listProvince' + countNumRow).css({ 'left': posEdgeRightMap + (countNumRow - 1) * widthLi })
      }
      countNumLine++
